...
#require_once '/opt/app-root/src/vendor/phpunit/phpunit/src/Framework/Assert/Functions.php';
...
class ApiFeatureContext implements Context
{
    ...
    /**
     * @Then /^the response status code should be with value (?P<code>\d+)$/
     */
    public function theResponseStatusCodeShouldBeWithValue($statusCode)
    {
        $response = $this->getLastResponse();
    ...
    {
        $this->scope = $scope;
        //throw new \Exception(sprintf("Group '%s' does not exist.", $scope));
    }
    /**
     * @Then /^reset scope$/
     */
    ...
    /**
     * @Given /^print last responses$/
     */
    public function printLastResponses()
    {
        if ($this->lastResponse) {
